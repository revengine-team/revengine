...
impl LayoutBuilder {
    ...
    pub fn filtering_sampler(self, visibility: wgpu::ShaderStages) -> Self {
        let ty = wgpu::BindingType::Sampler(SamplerBindingType::Filtering);
        self.binding(visibility, ty)
    }

    /// Add a non-filtering sampler binding to the layout.
    pub fn non_filtering_sampler(self, visibility: wgpu::ShaderStages) -> Self {
        let ty = wgpu::BindingType::Sampler(SamplerBindingType::Filtering);
        self.binding(visibility, ty)
    }

    /// Add a comparsion sampler binding to the layout.
...
impl<'a> Builder<'a> {
        ...
        T: Copy,
    {
        ...
        let size = NonZeroU64::new(end - start).expect("Buffer slice must not be empty");
